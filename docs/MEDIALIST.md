...
Bouton poubelle (icone 🗑️) affiché au survol de chaque carte, visible uniquement pour le propriétaire (`MEDIALIST_OWNER_EMAIL`). Appelle `DELETE /api/medialist/[id]` puis retire la carte de la liste localement.
...
Les stats sont rechargées automatiquement après chaque ajout ou suppression d'entrée. `refreshStats()` est appelé à la fin des handlers correspondants dans `index.vue`.
