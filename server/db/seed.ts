  ...
  await db.insert(experiences).values([
    {
      ...
      bulletsFr: [
        ...
        "Performance engineering : optimisation N+1 sur les mises à jour de commandes en masse — réduction de 61% du temps de réponse (1m47s → 42s sur 650 lignes, batch processing avec IN query + EnumMap UUID)",
        "Load testing : mise en place d'une suite k6 + Grafana Cloud (scénarios checkout V2/V3, search, cart — smoke tests et load tests, monitoring continu en preprod)",
      ...
      bulletsEn: [
        ...
        "Performance optimization: resolved N+1 query on bulk order updates — 61% response time reduction (1m47s → 42s on 650 lines, batch processing with IN query + EnumMap UUID)",
        "Load testing: set up k6 + Grafana Cloud suite (checkout V2/V3, search, cart scenarios — smoke tests and load tests, continuous preprod monitoring)",
      ...
      bulletsKm: [
        ...
        "Performance engineering៖ ដោះស្រាយ N+1 query លើការអាប់ដេតការបញ្ជាទិញជាបាច់ — កាត់បន្ថយ 61% នៃពេលវេលាឆ្លើយតប (1m47s → 42s លើ 650 បន្ទាត់ batch processing)",
        "Load testing៖ ដំឡើង k6 + Grafana Cloud (checkout V2/V3 search cart — smoke tests និង load tests ការត្រួតពិនិត្យ preprod ជាបន្ត)",
